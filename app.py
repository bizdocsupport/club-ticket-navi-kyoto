...
def display_table(
    ...
    base_config: dict[str, object] = {
        "試合日": st.column_config.TextColumn("試合日", width=120),
...
.ticket-main {{
  ...
  background:linear-gradient(90deg,{brand_primary} 0 68%,{brand_secondary} 68% 100%);
...
.ticket-stub {{
  ...
  background:linear-gradient(180deg,{brand_secondary} 0 35%,{brand_primary} 35% 100%);
}}
...
if news.empty:
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "掲載日": st.column_config.TextColumn("掲載日", width="small"),
